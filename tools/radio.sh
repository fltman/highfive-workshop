...
# PENGAR: ElevenLabs-kontot har övertrassering påslagen. Skriptet läser kontots RIKTIGA räknare före varje sändning och
...
RÖST="${RADIO_ROST:-6MuG7W2I4HAxP8rwUO6Z}"; RÖSTNAMN="${RADIO_ROSTNAMN:-Bengt}"; MODELL="${RADIO_MODELL:-eleven_v3}"
...
sänd() {
  före=$(kvar); if [ "$före" -le "$GOLV" ]; then echo "$(date +%H:%M:%S) STOPP: $före krediter kvar, golvet är $GOLV. Radion spelar bara musik nu."; return 2; fi
...
PROMPT
  ...
  python3 - "$W/svar.txt" "$MAXT" "$RÖSTNAMN" > "$W/seg.json" <<'PY' || { echo "$(date +%H:%M:%S) kunde inte tolka manuset"; return 1; }
import sys, json, re
...
PY
  ...
  kod=$(curl -s -o "$W/prat.mp3" -w '%{http_code}' -X POST "https://api.elevenlabs.io/v1/text-to-speech/$RÖST?output_format=mp3_44100_128" -H "xi-api-key: $XI" -H 'content-type: application/json' --data-binary @"$W/tts.json")
...
PY
  ...
  sleep 4; efter=$(kvar); echo "$(date +%H:%M:%S) i etern: $(python3 -c "import json; print(json.load(open('$W/seg.json')).get('titel',''))") · $(cat "$W/antal.txt") tecken, ${sek}s · kostade $((före-efter)) krediter, $efter kvar (golv $GOLV) · nästa låt: ${spela:-slumpad}"
}
if [ "${1:-}" = en ]; then sänd; exit $?; fi
while true; do sänd; [ $? -eq 2 ] && exit 0; sleep "$INTERVALL"; done

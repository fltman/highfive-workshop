...
Det här är ett lokalt agentteam som bidrar till gruppens gemensamma projekt, **Staden**. Teamet är byggt på labbet `agent-factory`: läs `CLAUDE.md` här i mappen, det är manualen för hur teamet organiserar sig.

## Uppdraget

**Kvarteret är valt: Elverket.** Specen står i `UPPDRAG.md` här i mappen — läs den före `/start`,
den innehåller redan svaren på CEO:s discovery-frågor. Ropat i `#bygge` [86].
...
3. Leverera med PR från branchen `team/lp` mot `main`. Skriv i PR-texten vad ni bidrar med och hur det syns.
4. Rör inte andra teams mappar eller filer. Vill ni ändra något gemensamt: PR och en rad i `#bygge`.
5. Ni får bygga **både frontend och backend**. Backend: `board/plugins/lp/index.js` monteras på `/t/lp/` och får ett API mot Torget (`board.post`, `board.query`, `onMessage`), se `board/plugins/README.md`. Frontend: `board/public/staden/kvarter/lp/index.html` (en katalog, lägg js/css/bilder bredvid) syns som er ruta på `/staden` och kan anropa er backend på samma origin. Båda dyker upp när PR:en mergats och deployats.
